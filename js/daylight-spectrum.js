...
const au = { version: 1, wgsl: "struct U{params:vec4f,pointer:vec4f,}@group(0) @binding(0) var<uniform> u:U;fn hash2(a:vec2f)-> f32{let b=dot(a,vec2f(127.1,311.7));return fract(sin(b)*43758.5453123);}fn vnoise(a:vec2f)-> f32{let b=floor(a);let c=fract(a);let d=c*c*(3.0-2.0*c);let e=hash2(b+vec2f(0.0,0.0));let f=hash2(b+vec2f(1.0,0.0));let g=hash2(b+vec2f(0.0,1.0));let h=hash2(b+vec2f(1.0,1.0));return mix(mix(e,f,d.x),mix(g,h,d.x),d.y);}fn fbm(a:vec2f)-> f32{var b=0.0;var c=0.5;var d=a;for(var e=0;e<5;e=e+1){b=b+c*vnoise(d);d=d*2.02+vec2f(11.3,7.7);c=c*0.5;}return b;}fn ramp(a:f32)-> vec3f{let b=clamp(a,0.0,1.0);let c=vec3f(0.988,0.878,0.686);let d=vec3f(0.988,0.780,0.420);let e=vec3f(0.988,0.612,0.176);let f=vec3f(0.965,0.435,0.000);let g=vec3f(1.000,0.620,0.180);let h=vec3f(0.992,0.835,0.325);let i=vec3f(1.000,0.972,0.870);var j=mix(c,d,smoothstep(0.00,0.22,b));j=mix(j,e,smoothstep(0.18,0.40,b));j=mix(j,f,smoothstep(0.36,0.56,b));j=mix(j,g,smoothstep(0.54,0.70,b));j=mix(j,h,smoothstep(0.68,0.86,b));j=mix(j,i,smoothstep(0.86,1.00,b));return j;}fn flow(a:vec2f,b:f32,c:f32)-> f32{let d=vec2f(-0.80,0.60);let e=vec2f(0.60,0.80);let f=dot(a,d);let g=dot(a,e);let h=fbm(a*1.35+vec2f(b*0.030,-b*0.020));let i=g+(h-0.5)*c;var j=sin(i*6.0-f*1.3+b*0.50+h*2.0);j=j+0.55*sin(i*11.0-f*0.8-b*0.34+h*3.0);j=j+0.30*sin(i*18.5+f*0.4+b*0.62);j=j/1.85;let k=pow(clamp(1.0-abs(j),0.0,1.0),1.6);return mix(j*0.5+0.5,k,0.72);}fn lattice(a:vec2f,b:f32)-> f32{let c=0.150;let d=floor(a/c);let e=fract(a/c)-vec2f(0.5,0.5);let f=abs(e.x)*2.0;let g=abs(e.y)*2.0;let h=smoothstep(0.62,1.0,max(f,g));let i=smoothstep(0.32,0.0,length(e));let j=0.5+0.5*sin(b*0.6+(d.x-d.y)*0.9+hash2(d)*6.28);return 0.30+h*0.62+i*0.30*j;}@fragment fn fs_main(@location(0) a:vec2f)-> @location(0) vec4f{let b=u.params.x;let c=clamp(u.params.y,0.0,1.0);let d=max(u.params.z,0.0001);var e=a-vec2f(0.5,0.5);e.x=e.x*d;let f=u.pointer.xy;let g=clamp(u.pointer.z,0.0,1.0);let h=e+f*(0.05+g*0.06);let i=mix(0.14,1.0,smoothstep(0.06,0.94,c));let j=mix(0.42,0.10,i);let k=flow(h,b,j);let l=lattice(h,b);var m=mix(k,l,i);let n=vec2f(0.62*d,-0.40);let o=length((h-n)*vec2f(0.72,1.0));m=m+smoothstep(1.35,0.0,o)*0.30;let p=length(e*vec2f(0.82,1.16));m=m*0.80+smoothstep(1.30,0.0,p)*0.30;let q=length(e-f*vec2f(0.45*d,0.45));m=m+smoothstep(0.55,0.0,q)*(0.10+g*0.16);m=m+(hash2(a*900.0+vec2f(b*60.0,0.0))-0.5)*0.018;var r=ramp(clamp(m,0.0,1.0));let s=smoothstep(1.55,0.35,length(e));r=r*mix(0.92,1.02,s);return vec4f(r,1.0);}" };
function Tn() {
  try {
